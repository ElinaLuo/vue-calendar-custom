        ...
        select(k1, k2, e) {
            ...
            if (this.range) {
                if (this.rangeBegin.length == 0 || this.rangeEndTemp != 0) {
                    ...
                    this.$emit('select',this.getFormatDate(begin),this.getFormatDate(end))
                }
                this.render(this.year, this.month)
            } else {
                // 取消上次选中
                ...
                this.today = [k1, k2]
                begin = [this.year,this.zero?this.zeroPad(this.month + 1):this.month + 1,this.zero?this.zeroPad(this.days[k1][k2].day):this.days[k1][k2].day]
                this.$emit('select', this.getFormatDate(begin))

            }
        },
        getFormatDate(datalist) {
            return datalist && datalist.join(this.separator)
        },
        // 日期补零
        zeroPad(n){
...
.calendar td:not(.selected) span:hover{
    background:#f3f8fa;
    color:#444;
}
